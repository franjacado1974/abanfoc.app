...
export default function RevisionChecklist() {
    ...
    const [centro, setCentro] = useState<Centro | null>(() => {
        try {
            const items: Centro[] = JSON.parse(localStorage.getItem('firecheck_db_centros') || '[]');
            return items.find((c: any) => c.id === centroId || c._docId === centroId) || null;
        } catch { return null; }
    });
    const [parte, setParte] = useState<Parte | null>(() => {
        try {
            const items: Parte[] = JSON.parse(localStorage.getItem('firecheck_db_partes') || '[]');
            return items.find((p: any) => p.id === parteId || p._docId === parteId) || null;
        } catch { return null; }
    });
    ...
    const [sistemasDelCentro, setSistemasDelCentro] = useState<CentroSistema[]>(() => {
        try {
            const all = JSON.parse(localStorage.getItem('firecheck_db_centro_sistemas') || '[]');
            return Array.isArray(all) ? all.filter((s: any) => s.centroId === centroId) : [];
        } catch { return []; }
    });
    const [equiposInstalados, setEquiposInstalados] = useState<EquipoInstalado[]>(() => {
        try {
            const all = JSON.parse(localStorage.getItem('firecheck_db_equipos_instalados') || '[]');
            return Array.isArray(all) ? all.filter((e: any) => e.centroId === centroId) : [];
        } catch { return []; }
    });
    ...
    const [loading, setLoading] = useState(() => {
        try {
            const items: Parte[] = JSON.parse(localStorage.getItem('firecheck_db_partes') || '[]');
            const found = items.find((p: any) => p.id === parteId || p._docId === parteId);
            return !found;
        } catch { return true; }
    });
    ...
    useEffect(() => {
        ...
        const loadLocalBundle = async () => {
            try {
                ...
                // Si ya tenemos parte y centro cargados desde localStorage o IndexedDB, asegurar que loading es false
                if (parteId) {
                    setLoading(prev => {
                        const localPartes = JSON.parse(localStorage.getItem('firecheck_db_partes') || '[]');
                        const exists = localPartes.some((p: any) => p.id === parteId || p._docId === parteId);
                        return exists ? false : prev;
                    });
                }
            }
        };
    ...
    useEffect(() => {
        ...
        // Timer de seguridad: liberar pantalla de carga tras 2.5s si Firestore tarda
        ...
        const unsubClientes = subscribeClientes((items) => {
            ...
            safeLocalStorageSet('firecheck_db_clientes', JSON.stringify(items));
        });
        ...
        const unsubCentros = subscribeCentros((items) => {
            ...
            safeLocalStorageSet('firecheck_db_centros', JSON.stringify(items));
        });
        ...
        const unsubPartes = subscribePartes((items) => {
            ...
            safeLocalStorageSet('firecheck_db_partes', JSON.stringify(items));
            setLoading(false);
        });
        ...
        const unsubSistemas = subscribeCentroSistemas(centroId, (items: CentroSistema[]) => {
            ...
            safeLocalStorageSet('firecheck_db_centro_sistemas', JSON.stringify(items));
        });
        ...
        const unsubSst = subscribeSistemasCategorias((items) => {
            ...
            safeLocalStorageSet('firecheck_db_sistemas_categorias', JSON.stringify(items));
        });
        ...
        const unsubArt = subscribeArticulos((items) => {
            ...
            safeLocalStorageSet('firecheck_db_sistemas_equipos', JSON.stringify(revisables));
        });
...
// Generar numeroMantenimiento si no existe
    ...
    const updateParte = (changes: Partial<Parte>) => {
        ...
        safeLocalStorageSet('firecheck_db_partes', JSON.stringify(updatedPartes));
    ...
    const handleFinalizarConFirmas = async () => {
        ...
        try {
            ...
            try {
                ...
                safeLocalStorageSet('firecheck_db_certificados', JSON.stringify(certificadosExistentes));
            } catch (certSaveErr) {
    ...
    const handleSaveComments = async () => {
        ...
        if (docId) {
            ...
            safeLocalStorageSet('firecheck_db_centros', JSON.stringify(updatedCentros));
    ...
    const confirmDeleteEquipo = async () => {
        ...
        safeLocalStorageSet('firecheck_db_equipos_instalados', JSON.stringify(updatedAll));
    ...
    const moveSistema = (sistemaId: string, direction: 'up' | 'down') => {
        ...
        safeLocalStorageSet('firecheck_revision_sistema_orden', JSON.stringify(newOrden));
    };
