...
import { db, subscribeEmpresas, moverAPapelera, subscribePresupuestos, subscribePedidos, type Presupuesto, type Pedido } from './firebase';
import { APP_VERSION } from './constants';
import jsPDF from 'jspdf';
import autoTable from 'jspdf-autotable';
import { fetchImageToBase64, optimizarImagenParaPDF, getImageFormat, generarPresupuestoPDF } from './pdfGenerator';
...
export default function PruebasTecnicas() {
  ...
  // Listas de empresas, presupuestos y pedidos para vincular PDV
  const [empresasLista, setEmpresasLista] = useState<any[]>([]);
  const [presupuestosLista, setPresupuestosLista] = useState<Presupuesto[]>([]);
  const [pedidosLista, setPedidosLista] = useState<Pedido[]>([]);

  // Suscripción a Firestore para empresas, presupuestos y pedidos en tiempo real
  useEffect(() => {
    try {
      ...
      const unsubPres = subscribePresupuestos((items) => setPresupuestosLista(items));
      const unsubPed = subscribePedidos((items) => setPedidosLista(items));
      return () => {
        unsubEmp();
        unsubPres();
        unsubPed();
      ...
      console.error('Error suscribiendo a empresas/presupuestos/pedidos:', e);
    }
  }, []);
  ...
  const handleDescargarPDV = async (ensayo: EnsayoGuardado) => {
    // 1. Buscar presupuesto o pedido vinculado
    const matchPresupuesto = presupuestosLista.find(p => 
      (ensayo.presupuestoId && (p.id === ensayo.presupuestoId || (p as any)._docId === ensayo.presupuestoId)) ||
      (ensayo.pedidoId && (
        p.numeroPresupuesto === ensayo.pedidoId ||
        `PDV ${p.numeroPresupuesto?.replace(/^(PDV|PRV|PRE)[-\s]*/i, '')}` === ensayo.pedidoId ||
        p.id === ensayo.pedidoId
      )) ||
      (ensayo.clienteNombre && p.nombreCliente && p.nombreCliente.toLowerCase().trim() === ensayo.clienteNombre.toLowerCase().trim())
    );

    const matchPedido = pedidosLista.find(ped => 
      (ensayo.pedidoId && (ped.numeroPedido === ensayo.pedidoId || ped.id === ensayo.pedidoId || (ped as any)._docId === ensayo.pedidoId)) ||
      (ensayo.presupuestoId && ped.presupuestoId === ensayo.presupuestoId) ||
      (ensayo.clienteNombre && (ped as any).nombreCliente && (ped as any).nombreCliente.toLowerCase().trim() === ensayo.clienteNombre.toLowerCase().trim())
    );

    const nombreCliente = ensayo.clienteNombre || matchPresupuesto?.nombreCliente || (matchPedido as any)?.nombreCliente || 'Cliente';

    // Empresa mantenedora
    let empresaSeleccionada = empresasLista.find(e => 
      (e.nombre && ensayo.empresaMantenedora && e.nombre.toLowerCase().trim() === ensayo.empresaMantenedora.toLowerCase().trim()) ||
      (matchPresupuesto && ((matchPresupuesto as any).empresaId === e.id || (matchPresupuesto as any).empresaId === e._docId)) ||
      (matchPedido && (matchPedido.empresaId === e.id || matchPedido.empresaId === e._docId))
    );
    if (!empresaSeleccionada && empresasLista.length > 0) {
      empresaSeleccionada = empresasLista[0];
    }

    // Código de pedido (ej: PDV XXXXXX)
    let codigoPDV = ensayo.pedidoId || matchPedido?.numeroPedido || '';
    if (!codigoPDV && matchPresupuesto?.numeroPresupuesto) {
      codigoPDV = `PDV ${matchPresupuesto.numeroPresupuesto.replace(/^(PDV|PRV|PRE)[-\s]*/i, '')}`;
    }
    if (!codigoPDV) {
      codigoPDV = `PDV-${ensayo.id.replace(/^[A-Z]+-/, '')}`;
    }
    ...
    if (matchPresupuesto?.lineas && matchPresupuesto.lineas.length > 0) {
      ...
      lineasPDF = [{
        concepto: `Prueba técnica: ${ensayo.equipoAMedirLabel || 'Ensayo hidráulico'} (Centro: ${ensayo.centroNombre || 'Instalación'} - Fecha: ${formatFechaEnsayo(ensayo.fechaEnsayo)})`,
        cantidad: 1,
        precioUnidad: 0,
        subtotal: 0
      }];
    ...
    const notas = matchPresupuesto?.notas || matchPedido?.notas || ensayo.observaciones || '';
    const fechaCreacion = matchPedido?.fechaCreacion || matchPresupuesto?.fechaCreacion || (ensayo.fechaEnsayo ? `${ensayo.fechaEnsayo}T10:00:00.000Z` : new Date().toISOString());

    await generarPresupuestoPDF({
      titulo: matchPresupuesto?.titulo || matchPedido?.titulo || `Prueba Técnica: ${ensayo.equipoAMedirLabel || 'Ensayo de Presión y Caudal'}`,
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 self-end sm:self-center shrink-0">
                        ...
                        <button
                          onClick={() => handleDescargarPDV(ensayo)}
                          className="flex items-center gap-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-800 text-xs font-bold py-2 px-2.5 rounded-xl border border-indigo-200 transition-all active:scale-95 cursor-pointer"
                          title="Ver / Descargar Pedido de Venta (PDV) asociado"
                        >
