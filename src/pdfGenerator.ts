    ...
    try {
      ...
      img.onload = () => {
        try {
          ...
          // Eliminar fondos blancos, grisáceos y casi blancos haciendo transparente el canal alfa
          for (let i = 0; i < data.length; i += 4) {
            ...
            // Si los 3 canales RGB son altos (casi blanco/blanco) y con poca saturación (tono neutro de fondo)
            if (minVal > 215 && (maxVal - minVal) < 25) {
              data[i + 3] = 0; // Transparente total
            } else if (minVal > 195 && (maxVal - minVal) < 18) {
              // Suavizado en bordes cercanos al blanco
              const factor = (minVal - 195) / 20;
...
export const generarCertificadoPDF = async (
  ...
  // Para Certificados Oficiales, la empresa mantenedora y logotipos son siempre de ABANFOC
  const abanfocEmp = cargaDatosEmpresa('ABANFOC S.L.') || cargaDatosEmpresa('ABANFOC') || cargaDatosEmpresa('abanfoc');
  const empData = normalizarDatosEmpresa(abanfocEmp || 'ABANFOC S.L.');
  const margen = 14;
  ...
  // Logo a la derecha dentro de la tarjeta (siempre logotipo oficial de Abanfoc)
  try {
    const logoUrl = empData?.logoUrl || '/logo.png';
    const logoRaw = await fetchImageToBase64(logoUrl);
    if (logoRaw) {
      const { base64: logoData, format } = await optimizarImagenParaPDF(logoRaw, 800, 0.75);
      const logoProps = doc.getImageProperties(logoData);
...
export const generarPresupuestoPDF = async (
  ...
  const margen = 16;
  // Los presupuestos y pedidos de venta SIEMPRE se emiten con el nombre, datos oficiales y logo de ABANFOC S.L.
  let abanfocData: Record<string, any> | null = null;
  try {
    const rawEmpresas = typeof localStorage !== 'undefined' ? localStorage.getItem('firecheck_db_empresas') : null;
    if (rawEmpresas) {
      const listEmp: any[] = JSON.parse(rawEmpresas);
      abanfocData = listEmp.find(e => e?.nombre && e.nombre.toLowerCase().includes('abanfoc')) || null;
    }
  } catch (e) {
    console.error('Error buscando empresa ABANFOC en base de datos local:', e);
  }
  const empData = normalizarDatosEmpresa(abanfocData || cargaDatosEmpresa('abanfoc') || { nombre: 'ABANFOC S.L.' });

  // Sin marco exterior para mayor amplitud y margen limpio
  let y = 14;

  // ─────────────────────────────────────────────────────────────────────────
  // 1. CABECERA: [LOGO] | [DATOS EMPRESA] | [FORMAS CONTACTO] (2 líneas rojas)
  // ─────────────────────────────────────────────────────────────────────────
  const margenHeader = 12;
  const headerTopY = 5;
  const headerBottomY = 32;

  // Altura acortada de las líneas rojas (más pequeñas y estilizadas)
  const lineRedTopY = 9.5;
  const lineRedBottomY = 25.5;

  // Cargar y dibujar Logo oficial de ABANFOC (a la izquierda - extra grande)
  try {
    let logoData: string | null = null;
    // 1º Prioridad: Logo directo de la empresa ABANFOC en base de datos si existe
    if (abanfocData?.logoUrl) {
      logoData = await fetchImageToBase64(abanfocData.logoUrl);
    }
    // 2º Prioridad: Logo oficial de la aplicación en public (/logo.png)
    if (!logoData) {
      const fetched = await fetchImageToBase64('/logo.png');
      if (fetched) logoData = fetched;
    }
    // 3º Prioridad: Logo almacenado en caché de sesión/local si no hay otro
    if (!logoData) {
      const storedLogo = typeof localStorage !== 'undefined' ? localStorage.getItem('firecheck_db_logo') : null;
    ...
    console.error('Error cargando logo en presupuesto:', e);
  }

  // 1ª LÍNEA ROJA VERTICAL (separada más a la derecha para dar espacio al logo grande)
  ...
  const empLoc = (abanfocData?.poblacion && abanfocData?.provincia)
    ? `${abanfocData.poblacion}, ${abanfocData.provincia}`
    : 'Sta. Coloma Gramanet, Barcelona';
  const empRasic = abanfocData?.rasic || empData?.rasic || '10600168';

  const colEmpresaX = 88;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  ...
  const empEmail = abanfocData?.email || abanfocData?.correo || 'abanfoc@abanfoc.es';

  const colContactoX = 150;
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(6.7);
  doc.setTextColor(70, 75, 85);
  ...
  // ─────────────────────────────────────────────────────────────────────────
  const esPdv = !!presupuesto.esPedido || (presupuesto.numeroPresupuesto && presupuesto.numeroPresupuesto.toUpperCase().startsWith('PDV'));
  const tituloEncabezado = presupuesto.tituloDocumento || (esPdv ? 'PEDIDO DE VENTA' : 'PRESUPUESTO');

  y = 37;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(16);
  doc.setTextColor(20, 20, 25);
  doc.text(tituloEncabezado, pageWidth / 2, y, { align: 'center' });

  // Descripción del documento justo debajo del título (no negrita)
  const descripcionPresupuesto = (presupuesto.titulo || '').trim();
  if (descripcionPresupuesto) {
    y += 5.5;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(60, 65, 75);
    doc.text(descripcionPresupuesto.toUpperCase(), pageWidth / 2, y, { align: 'center' });
  }

  y += 7;

  // ─────────────────────────────────────────────────────────────────────────
  // 2. DATOS DE PRESUPUESTO / PEDIDO Y CLIENTE SEPARADOS POR LÍNEA ROJA VERTICAL
  // ─────────────────────────────────────────────────────────────────────────
  const colStartY = y;
  const colMidX = 104;

  // Columna Izquierda: DATOS DE PRESUPUESTO / PEDIDO DE VENTA
  const rawNum = presupuesto.numeroPresupuesto || '';
  const numLimpio = rawNum.replace(/^(PRV|PDV|PRE)[-\s]*/i, '').replace('.', '');
  const prefijoDoc = esPdv ? 'PDV' : 'PRV';
  const numDoc = numLimpio ? `${prefijoDoc}${numLimpio}` : (esPdv ? 'PDV263326' : 'PRV263326');

  const fechaDoc = presupuesto.fechaCreacion 
    ? new Date(presupuesto.fechaCreacion).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' })
    : '03/09/2026';

  const cuentaBanco = empData?.cuenta || empData?.iban || 'ES2800810039810001554059';

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.2);
  doc.setTextColor(220, 38, 38);
  const tituloSeccionDatos = esPdv ? 'DATOS DE PEDIDO DE VENTA' : 'DATOS DE PRESUPUESTO DE VENTA';
  doc.text(tituloSeccionDatos, margen, colStartY);

  // Línea gris sutil bajo título de presupuesto / pedido
  doc.setDrawColor(220, 225, 235);
  doc.setLineWidth(0.3);
  doc.line(margen, colStartY + 1.5, margen + (esPdv ? 58 : 55), colStartY + 1.5);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.setTextColor(70, 75, 85);
  doc.text('Número: ', margen, colStartY + 5.5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(20, 20, 20);
  doc.text(numDoc, margen + 14, colStartY + 5.5);

  doc.setFont('helvetica', 'normal');
  doc.setTextColor(70, 75, 85);
  doc.text('Fecha: ', margen, colStartY + 9.5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(20, 20, 20);
  doc.text(fechaDoc, margen + 11, colStartY + 9.5);

  doc.setFont('helvetica', 'normal');
  doc.setTextColor(70, 75, 85);
  doc.text('Forma de pago: ', margen, colStartY + 13.5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(20, 20, 20);
  doc.text('Transferencia bancaria', margen + 23, colStartY + 13.5);

  doc.setFont('helvetica', 'normal');
  doc.setTextColor(70, 75, 85);
  doc.text('Cuenta: ', margen, colStartY + 17.5);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(20, 20, 20);
  doc.text(cuentaBanco, margen + 13, colStartY + 17.5);

  // BARRA VERTICAL ROJA SEPARADORA (más corta y estilizada)
  doc.setDrawColor(220, 38, 38);
  doc.setLineWidth(0.45);
  doc.line(colMidX, colStartY + 2.5, colMidX, colStartY + 25.5);

  // Columna Derecha: DATOS DEL CLIENTE (más a la derecha)
  const cli = presupuesto.cliente || {};
  const cliNombre = (presupuesto.nombreCliente && presupuesto.nombreCliente !== 'Cliente') 
    ? presupuesto.nombreCliente 
    : (cli.nombre || 'PREVENCIÓN Y AYUDA CONTRA INCENDIOS S.L');
  const cliCodVal = cli.codigoCliente || cli.codigo || cli.id;
  const cliCodigo = cliCodVal ? `Nº CLIENTE ${cliCodVal}` : 'Nº CLIENTE CLI00018';
  const cliCif = cli.cif ? `${cli.cif}` : 'B-83846055';
  const cliDir = cli.direccion ? `${cli.direccion}` : 'Calle Joaquín Sorolla 4';
  const cliLocParts = [cli.cp, cli.poblacion, cli.provincia, 'España'].filter(Boolean);
  const cliLoc = cliLocParts.length > 1 ? cliLocParts.join(', ') : '28522, Rivas Vaciamadrid, Madrid, España';
  const cliTel = cli.telefono ? `Telf: ${cli.telefono}` : 'Telf: 914990499';

  const colCliX = colMidX + 16;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.2);
  doc.setTextColor(220, 38, 38);
  doc.text('DATOS DEL CLIENTE', colCliX, colStartY);

  // Línea gris sutil bajo título de cliente (más corta)
  doc.setDrawColor(220, 225, 235);
  doc.setLineWidth(0.3);
  doc.line(colCliX, colStartY + 1.5, colCliX + 40, colStartY + 1.5);

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(20, 20, 20);
  doc.text(cliNombre, colCliX, colStartY + 5.5);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  doc.text(cliCodigo, colCliX, colStartY + 9.5);
  doc.text(cliCif, colCliX, colStartY + 13.5);
  doc.text(cliDir, colCliX, colStartY + 17.5);
  doc.text(cliLoc, colCliX, colStartY + 21.5);
  doc.text(cliTel, colCliX, colStartY + 25.5);

  // ─────────────────────────────────────────────────────────────────────────
  // 3. TABLA DE ARTÍCULOS:
  // Encabezados: Código | Artículo | Precio | Unidades | Subtotal
  // Subrayados por la línea gris horizontal
  // ─────────────────────────────────────────────────────────────────────────
    ...
    for (const l of lineasConFoto) {
      if (l.fotoUrl) {
        try {
          ...
          if (base64) {
            // Eliminar fondo blanco y recortar a transparencia limpia
            const sinFondo = await removeWhiteBackground(base64, 400);
            // Optimizar manteniendo PNG con canal alfa transparente
            const { base64: base64Opt } = await optimizarImagenParaPDF(sinFondo, 600, 0.8);
  ...
  autoTable(doc, {
    ...
    body: tableBody,
    theme: 'plain', // Sin fondos pesados ni líneas verticales
    headStyles: {
      fillColor: [255, 255, 255], // Fondo blanco limpio encima de la línea gris
      textColor: [25, 30, 40],
      fontStyle: 'bold',
      fontSize: 8,
      ...
      if (data.section === 'body' && data.column.index === 0 && colFotoWidth > 0) {
        ...
        if (imgBase64) {
          try {
            ...
            const posX = data.cell.x + (cellW - finalW) / 2;
            const posY = data.cell.y + (cellH - finalH) / 2;
            doc.addImage(imgBase64, 'PNG', posX, posY, finalW, finalH);
          } catch (e) {
            console.error('Error dibujando foto artículo:', e);
          }
        }
      }
  ...
  if (ivaExento) {
    ...
    const exencionH = 8 + exencionSplit.length * 4;
    doc.roundedRect(margen, currentBottomY, pageWidth - margen * 2, exencionH, 2.5, 2.5, 'FD');
    doc.setFont('helvetica', 'italic');
    doc.setFontSize(7);
    doc.setTextColor(170, 110, 30);
    let exy = currentBottomY + 5;
    exencionSplit.forEach((line: string) => { doc.text(line, margen + 4, exy); exy += 4; });
    currentBottomY += exencionH + 6;
  }

  // ─────────────────────────────────────────────────────────────────────────
  // ─────────────────────────────────────────────────────────────────────────
  // RECUADRO DE ACEPTACIÓN CON FECHA Y FIRMA (a la izquierda) Y NOTAS (a la derecha)
  // Anclado abajo del todo, con altura generosa para albergar sello y firma
  // ─────────────────────────────────────────────────────────────────────────
  const firmaBoxW = 74;
  ...
  // NOTAS Y CONDICIONES (a la derecha del recuadro de aceptación)
  const notasX = margen + firmaBoxW + 6;
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    // Cláusula de protección de datos (RGPD) alineada a la izquierda
    const textoRgpd = 'De acuerdo a lo establecido en la normativa de protección de datos, ABANFOC SL con CIF B16794679, le informa que tratará sus datos con la finalidad de llevar a cabo la gestión administrativa, facturación, contabilidad y obligaciones legales. Podrá ejercer sus derechos dirigiéndose a la dirección postal C/AMERICA, 16 B 08921 SANTA COLOMA DE GRAMENET (BARCELONA) o a través de abanfoc@abanfoc.es. Para información adicional podrá diríjase a abanfoc@abanfoc.es.';
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(4.8);
    doc.setTextColor(140, 145, 155);
    const anchoRgpd = pageWidth - margen * 2 - 22; // Deja espacio para la barra y la paginación a la derecha
    const rgpdSplit = doc.splitTextToSize(textoRgpd, anchoRgpd);
    const footerTopY = pageHeight - 16;
    let rgpdY = footerTopY;
    rgpdSplit.forEach((line: string) => {
      doc.text(line, margen, rgpdY);
      rgpdY += 2.2;
    });

    // Barra separadora vertical desplazada a la derecha
    const separatorX = pageWidth - margen - 18;
    doc.setDrawColor(210, 215, 225);
    doc.setLineWidth(0.35);
    doc.line(separatorX, footerTopY - 1, separatorX, footerTopY + 9);

    // Número de página alineado a la derecha en el margen derecho
    const rightMarginX = pageWidth - margen;
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(6.8);
    doc.setTextColor(120, 125, 135);
    doc.text(`Pág. ${i} / ${totalPages}`, rightMarginX, footerTopY + 5, { align: 'right' });

    // Texto legal en vertical en el margen izquierdo del documento (bajado hacia la esquina inferior)
    const textoRegistro = 'ABANFOC S.L. Empresa inscrita en el registro mercantil de Barcelona, Tomo 48030, Folio 80 , Hoja 570011, Sección general Inscripción 1';
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(5.5);
    doc.setTextColor(115, 120, 130);
    // Bajado en el lateral izquierdo, iniciando en Y = 220 (+15 puntos) y extendiéndose hacia abajo
    doc.text(textoRegistro, 5.5, 220, { angle: 90 });
  }

  const filePrefix = esPdv ? 'Pedido' : 'Presupuesto';
  doc.save(`${filePrefix}_${presupuesto.numeroPresupuesto || 'N-A'}.pdf`);
};