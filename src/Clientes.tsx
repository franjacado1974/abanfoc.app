...
export default function Clientes() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!newId) {
      let maxNum = 0;
      clientes.forEach(c => {
        if (typeof c.id === 'string' && c.id.startsWith('CLI')) {
  ...
  // Obtener el siguiente ID previsto si estamos creando uno nuevo
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button onClick={() => { setForm(emptyCliente); setView('form'); }} className="flex items-center gap-2 bg-black text-white px-5 py-2.5 rounded-xl font-medium hover:bg-zinc-800 transition-all text-sm shadow-md shadow-black/10">
          ...
          {clientes.length === 0 ? (
            <div className="bg-white rounded-[2rem] border border-zinc-200 p-16 text-center shadow-sm">
              ...
              <button onClick={() => { setForm(emptyCliente); setView('form'); }} className="inline-flex items-center gap-2 bg-black text-white px-6 py-3 rounded-xl font-medium hover:bg-zinc-800 transition-all shadow-md">
  ...
  return (
    <div className="min-h-screen bg-blue-50/40 p-6 md:p-12">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-black text-white p-5 rounded-xl flex items-center justify-between mb-8">
          <div>
            <p className="text-xs text-zinc-400 font-bold mb-1">CÓDIGO DE CLIENTE AUTOGENERADO</p>
            <p className="text-xl font-mono font-bold tracking-wider">{nextIdPreview}</p>
          </div>
        </div>
