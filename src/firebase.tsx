...
export interface PresupuestoSeguimientoEvento {
  tipo: 'envio' | 'visita' | 'descarga_pdf' | 'firmado' | 'rechazado';
  fecha: string;
  detalle?: string;
}
...
export interface PresupuestoSeguimiento {
  ...
  eventos?: PresupuestoSeguimientoEvento[];
}
...
export interface Presupuesto {
  ...
  _docId?: string;
  seguimiento?: PresupuestoSeguimiento;
  pedidoId?: string;
  tipoTrabajo?: 'Reparación' | 'Instalación' | 'Prueba técnica' | string;
}
...
export async function addPedido(pedido: Pedido) {
  try {
    const pedidoToSave = cleanFirestoreData({ ...pedido, updatedAt: new Date().toISOString() });
...
export async function updatePedido(id: string, pedido: Partial<Pedido>) {
  try {
    ...
    const dataToSave = cleanFirestoreData({ ...pedido, updatedAt: new Date().toISOString() });
...
export async function updatePresupuesto(id: string, presupuesto: Partial<Presupuesto>) {
  try {
    let docRef = doc(db, 'presupuestos', id);
    const snap = await getDoc(docRef);
    if (!snap.exists()) {
      const col = collection(db, 'presupuestos');
  ...
  try {
    ...
    const unsub = onSnapshot(col, (snap) => {
      ...
      return {
        ...
        descuentoPorcentaje: typeof data?.descuentoPorcentaje === 'number' ? data.descuentoPorcentaje : (Number(data?.descuentoPorcentaje) || 0),
        descuentoImporte: typeof data?.descuentoImporte === 'number' ? data.descuentoImporte : (Number(data?.descuentoImporte) || 0),
        iva: typeof data?.iva === 'number' ? data.iva : 21,
        total: typeof data?.total === 'number' ? data.total : 0,
        notas: data?.notas || '',
        usuarioRealizado: data?.usuarioRealizado || '',
        seguimiento: data?.seguimiento || undefined,
...
export async function getPresupuestoByToken(token: string): Promise<Presupuesto | null> {
  try {
    if (!token) return null;
    const col = collection(db, 'presupuestos');
    // 1. Buscar por tokenAcceso de seguimiento
    const q = query(col, where('seguimiento.tokenAcceso', '==', token));
    const snap = await getDocs(q);
    if (!snap.empty) {
      const d = snap.docs[0];
      const data = d.data() as any;
      return { _docId: d.id, id: data?.id ?? d.id, ...data } as Presupuesto;
    }
    // 2. Fallback: buscar por ID directo del documento
    const docRef = doc(db, 'presupuestos', token);
    const docSnap = await getDoc(docRef);
    if (docSnap.exists()) {
      const data = docSnap.data() as any;
      return { _docId: docSnap.id, id: data?.id ?? docSnap.id, ...data } as Presupuesto;
    }
    return null;
  } catch (e) {
    console.error('getPresupuestoByToken error:', e);
    return null;
  }
}

export async function registrarVisitaPresupuesto(docId: string, seguimientoActual?: PresupuestoSeguimiento) {
  try {
    const ahora = new Date().toISOString();
    const prevVisitas = Number(seguimientoActual?.visitasCount) || 0;
    const nuevaVisitaCount = prevVisitas + 1;
    const primeraApertura = seguimientoActual?.primeraApertura || ahora;
    const eventos: PresupuestoSeguimientoEvento[] = Array.isArray(seguimientoActual?.eventos) ? [...seguimientoActual.eventos] : [];
    
    eventos.unshift({
      tipo: 'visita',
      fecha: ahora,
      detalle: prevVisitas === 0 ? 'Primera apertura del enlace por el cliente' : `Visita número ${nuevaVisitaCount}`
    });

    const updatedSeguimiento: PresupuestoSeguimiento = {
      ...(seguimientoActual || {}),
      primeraApertura,
      ultimaApertura: ahora,
      visitasCount: nuevaVisitaCount,
      eventos: eventos.slice(0, 50)
    };

    const docRef = doc(db, 'presupuestos', docId);
    await setDoc(docRef, { seguimiento: updatedSeguimiento, updatedAt: ahora }, { merge: true });
    return updatedSeguimiento;
  } catch (e) {
    console.error('registrarVisitaPresupuesto error:', e);
    return null;
  }
}

export async function registrarDescargaPresupuesto(docId: string, seguimientoActual?: PresupuestoSeguimiento) {
  try {
    const ahora = new Date().toISOString();
    const prevDescargas = Number(seguimientoActual?.descargasCount) || 0;
    const nuevaDescargasCount = prevDescargas + 1;
    const eventos: PresupuestoSeguimientoEvento[] = Array.isArray(seguimientoActual?.eventos) ? [...seguimientoActual.eventos] : [];

    eventos.unshift({
      tipo: 'descarga_pdf',
      fecha: ahora,
      detalle: `Documento PDF descargado (${nuevaDescargasCount}ª descarga)`
    });

    const updatedSeguimiento: PresupuestoSeguimiento = {
      ...(seguimientoActual || {}),
      descargasCount: nuevaDescargasCount,
      eventos: eventos.slice(0, 50)
    };
    ...
    console.error('registrarDescargaPresupuesto error:', e);
    return null;
  }
}

export async function firmarPresupuestoPublico(
  docId: string, 
  datosFirma: { nombre: string; dni: string; firmaBase64: string },
  seguimientoActual?: PresupuestoSeguimiento
) {
  try {
    const ahora = new Date().toISOString();
    const eventos: PresupuestoSeguimientoEvento[] = Array.isArray(seguimientoActual?.eventos) ? [...seguimientoActual.eventos] : [];
    ...
    eventos.unshift({
      tipo: 'firmado',
      fecha: ahora,
      detalle: `Presupuesto aceptado y firmado por ${datosFirma.nombre} (${datosFirma.dni})`
    });
    ...
    const updatedSeguimiento: PresupuestoSeguimiento = {
      ...
      firmado: {
        ...
        dni: datosFirma.dni,
        fecha: ahora,
        firmaBase64: datosFirma.firmaBase64
      },
      eventos: eventos.slice(0, 50)
    };
    ...
    await setDoc(docRef, { 
      estado: 'Aprobado', 
      seguimiento: updatedSeguimiento, 
      updatedAt: ahora 
    }, { merge: true });

    return true;
  } catch (e) {
    console.error('firmarPresupuestoPublico error:', e);
    throw e;
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIGURACION INICIO (LOADER) - Firestore
// ─────────────────────────────────────────────────────────────────────────────

export function subscribeLoaderConfig(callback: (config: { imagenUrl?: string; escala?: number } | null) => void) {
  try {
    const col = collection(db, 'configuracion_inicio');
    const unsub = onSnapshot(col, (snap) => {
      if (snap.empty) {
        callback(null);
        return;
      ...
      callback({ 
        ...
        escala: typeof d?.escala === 'number' ? d.escala : 100 
      });
    }, (err) => {
      console.error('subscribeLoaderConfig error:', err);
      callback(null);
    });
    return unsub;
  } catch (e) {
    console.error('subscribeLoaderConfig error:', e);
    return () => {};
  }
}

export async function saveLoaderConfig(config: { imagenUrl: string; escala?: number }) {
  try {
    const col = collection(db, 'configuracion_inicio');
    const snap = await getDocs(col);
    const data = {
      imagenUrl: config.imagenUrl,
      escala: typeof config.escala === 'number' ? config.escala : 100,
      updatedAt: new Date().toISOString()
    };
    if (snap.empty) {
      const ref = await addDoc(col, data);
      ...
      const ref = doc(db, 'configuracion_inicio', snap.docs[0].id);
      await setDoc(ref, data, { merge: true });
      return { _docId: snap.docs[0].id, ...data };
    }
  } catch (e) {
    console.error('saveLoaderConfig error:', e);
    throw e;
  }
}
...
export async function addReparacion(data: ReparacionItem) {
  try {
    const col = collection(db, 'reparaciones');
    const dataToSave = cleanFirestoreData({ ...data, updatedAt: new Date().toISOString() });
    const docRef = await addDoc(col, dataToSave);
    return { ...dataToSave, _docId: docRef.id };
  } catch (e) {
...
export function subscribeUrgencias(callback: (items: UrgenciaItem[]) => void) {
  try {
    const col = collection(db, 'urgencias');
    const unsub = onSnapshot(col, (snap) => {
      const items = snap.docs.map(d => {
        const data = d.data() as any;
        return {
      ...
      console.error('subscribeUrgencias error:', err);
      callback([]);
    });
    return unsub;
  } catch (e) {
    console.error('subscribeUrgencias error:', e);
    return () => {};
  }
}

export async function addUrgencia(data: UrgenciaItem) {
  try {
    const col = collection(db, 'urgencias');
    const dataToSave = cleanFirestoreData({ ...data, updatedAt: new Date().toISOString() });
    const docRef = await addDoc(col, dataToSave);
    ...
    console.error('addUrgencia error:', e);
    throw e;
  }
}

export async function updateUrgencia(docId: string, data: Partial<UrgenciaItem>) {
  try {
    const docRef = doc(db, 'urgencias', docId);
    const dataToSave = cleanFirestoreData({ ...data, updatedAt: new Date().toISOString() });
    await updateDoc(docRef, dataToSave as any);
  } catch (e) {
    console.error('updateUrgencia error:', e);
    throw e;
  }
}

export async function deleteUrgencia(docId: string) {
  try {
    const docRef = doc(db, 'urgencias', docId);
    await deleteDoc(docRef);
  } catch (e) {
    console.error('deleteUrgencia error:', e);
    throw e;
  }
}
