...
import { 
  subscribeInstalaciones, createInstalacion as addInstalacion, updateInstalacion, deleteInstalacion 
...
export default function Instalaciones() {
  ...
  const instalacionesFiltradas = instalaciones.filter(i => {
    ...
    if (search.trim()) {
      ...
      return matchIns || matchLugar || matchTec || matchCom || matchNota;
    }
    return true;
  });

  // Estadísticas
  const totalCount = instalaciones.length;
  const pendientesCount = instalaciones.filter(i => i.estado === 'Pendiente').length;
  const enCursoCount = instalaciones.filter(i => i.estado === 'En curso').length;
  const paradosCount = instalaciones.filter(i => i.estado === 'Parado').length;
  const finalizadosCount = instalaciones.filter(i => i.estado === 'Finalizado').length;
              ...
              {loading ? (
                ...
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-slate-400 font-medium">
                    No se encontraron proyectos de instalación.
                  </td>
                </tr>
              ) : (
