...
import {
  Building2, Users, ShieldCheck, FireExtinguisher, Plus, Trash2, ArrowLeft, Image as ImageIcon, X, Loader, Edit, Percent, ClipboardList, ChevronDown, ChevronUp, Sparkles, Upload, RotateCw
} from 'lucide-react';
import { addUserToFirestore, subscribeSistemasCategorias, addSistemaCategoria, deleteSistemaCategoria, updateSistemaCategoria, uploadFile, subscribeImpuestos, saveImpuestoConfig, subscribeTecnicos, saveTecnico, deleteTecnico, subscribeLoaderConfig, saveLoaderConfig } from '../../recursos-compartidos/firebase/firebase';
...
export default function Ajustes() {
  ...
  const [view, setView] = useState<'menu' | 'tecnicos' | 'usuarios' | 'sistemas' | 'impuestos' | 'plantillas' | 'loader_image'>('menu');
  ...
  // ─── GESTION DE IMAGEN INICIO (LOADER) ──────────────────────────────────
  const [loaderImage, setLoaderImage] = useState<string>(() => {
    try {
      return localStorage.getItem('firecheck_loader_image') || '/loader-racor.png';
    } catch {
      return '/loader-racor.png';
    }
  });
  const [loaderEscala, setLoaderEscala] = useState<number>(() => {
    try {
      const stored = localStorage.getItem('firecheck_loader_escala');
  ...
  const [loaderStatus, setLoaderStatus] = useState<'idle' | 'saving' | 'success' | 'error'>('idle');

  useEffect(() => {
    const unsub = subscribeLoaderConfig((config) => {
      if (config?.imagenUrl) {
        setLoaderImage(config.imagenUrl);
        localStorage.setItem('firecheck_loader_image', config.imagenUrl);
      }
      if (typeof config?.escala === 'number') {
        setLoaderEscala(config.escala);
        localStorage.setItem('firecheck_loader_escala', String(config.escala));
      }
    });
    return () => unsub();
  }, []);

  const handleLoaderFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    setLoaderFile(file);
    const reader = new FileReader();
    reader.onloadend = () => {
      setLoaderPreview(reader.result as string);
    };
    reader.readAsDataURL(file);
  };

  const handleSaveLoaderImage = async () => {
    setLoaderStatus('saving');
    try {
      let finalUrl = loaderPreview || loaderImage;
      if (loaderFile) {
        const path = `configuracion_inicio/loader_${Date.now()}`;
        finalUrl = await uploadFile(loaderFile, path);
      }
      await saveLoaderConfig({ imagenUrl: finalUrl, escala: loaderEscala });
      setLoaderImage(finalUrl);
      localStorage.setItem('firecheck_loader_image', finalUrl);
      localStorage.setItem('firecheck_loader_escala', String(loaderEscala));
      setLoaderFile(null);
      setLoaderPreview(null);
      setLoaderStatus('success');
      setTimeout(() => setLoaderStatus('idle'), 2500);
    } catch (err) {
      console.error('Error al guardar imagen de inicio:', err);
      setLoaderStatus('error');
      setTimeout(() => setLoaderStatus('idle'), 3000);
    }
  };

  const handleResetLoaderImage = async () => {
    setLoaderStatus('saving');
    try {
      const defaultUrl = '/loader-racor.png';
      await saveLoaderConfig({ imagenUrl: defaultUrl, escala: 100 });
      setLoaderImage(defaultUrl);
      setLoaderEscala(100);
      localStorage.setItem('firecheck_loader_image', defaultUrl);
      localStorage.setItem('firecheck_loader_escala', '100');
      setLoaderFile(null);
      setLoaderPreview(null);
      setLoaderStatus('success');
      setTimeout(() => setLoaderStatus('idle'), 2500);
    } catch (err) {
      console.error('Error al restablecer imagen:', err);
      setLoaderStatus('error');
      setTimeout(() => setLoaderStatus('idle'), 3000);
    }
  };
    ...
    <div className="min-h-screen bg-[#F8FAFC] px-8 py-6">
      ...
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-black text-zinc-950 tracking-tight">
          {view === 'menu' ? 'Configuración de Salamandra' : view === 'tecnicos' ? 'Gestión de Técnicos' : view === 'usuarios' ? 'Gestión de Usuarios' : view === 'sistemas' ? 'Gestión de Sistemas' : view === 'plantillas' ? 'Editor de Plantillas' : view === 'loader_image' ? 'Cambio de imagen inicio' : 'Tipos de Impuestos'}
        </h1>
        <p className="text-xs font-semibold text-zinc-500 mt-1">
          {view === 'menu' 
            ? 'Administración de técnicos, usuarios, sistemas, plantillas, imagen de inicio e impuestos de Salamandra.' 
            : view === 'tecnicos' 
            ...
            : view === 'loader_image'
            ? 'Personaliza la imagen o logo que gira durante la pantalla de carga inicial.'
      ...
      <div className={view === 'plantillas' ? 'w-full' : view === 'menu' ? 'max-w-5xl' : 'max-w-2xl'}>
        {view === 'menu' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <button
              onClick={() => setView('loader_image')}
              className="w-full flex flex-col items-center justify-center text-center p-6 min-h-[220px] rounded-3xl border border-zinc-200 bg-white hover:border-rose-250 hover:bg-rose-50/30 transition-all group cursor-pointer shadow-sm hover:shadow-md active:scale-95"
            >
              <div className="w-16 h-16 bg-rose-100 text-rose-600 rounded-2xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform shadow-sm">
                <RotateCw className="w-7 h-7" />
              </div>
              <div>
                <p className="font-extrabold text-zinc-900 text-base">Cambio de imagen inicio</p>
                <p className="text-xs text-zinc-500 mt-1.5 max-w-[200px] leading-relaxed">Cargar imagen giratoria para la pantalla de inicio.</p>
              </div>
            </button>
          </div>
        )}
          ...
          <section className="space-y-6">
            <div className="bg-white rounded-3xl border border-zinc-200 p-6 sm:p-8 shadow-sm">
              <div className="max-w-md mx-auto flex flex-col items-center text-center">
                {/* Visualizador interactivo idéntico al inicio */}
                <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-4">
                  Vista previa en tiempo real
                </p>

                <div className="relative w-44 h-44 mb-6 bg-zinc-950 rounded-full flex items-center justify-center border border-zinc-900 shadow-[inset_0_2px_4px_rgba(0,0,0,0.5),0_10px_25px_-5px_rgba(239,68,68,0.25)] overflow-hidden">
                  {/* Outer spinning dashed ring */}
                  <div className="absolute inset-2 border-2 border-dashed border-red-500/40 rounded-full animate-spin-slow origin-center"></div>
                  
                  {/* Rotating Image Container */}
                  <div className="w-28 h-28 flex items-center justify-center animate-spin-slow origin-center">
                    <img 
                      src={loaderPreview || loaderImage} 
                      alt="Vista previa" 
                      style={{ transform: `scale(${loaderEscala / 100})` }}
                      className="w-full h-full aspect-square object-contain select-none pointer-events-none filter drop-shadow-[0_0_12px_rgba(255,255,255,0.25)] transition-transform duration-100"
                      onError={(e) => { (e.target as HTMLImageElement).src = '/loader-racor.png'; }}
                    />
                  </div>
                </div>
                ...
                <p className="text-sm font-semibold text-zinc-700 mb-1">
                  {loaderFile ? 'Nueva imagen seleccionada (pendiente de guardar)' : 'Imagen de carga activa'}
                </p>
                <p className="text-xs text-zinc-400 mb-5">
                  Se recomienda una imagen o logotipo en formato PNG con fondo transparente (o JPG centrado).
                </p>

                {/* Control de Tamaño / Zoom para ajustar al círculo */}
                <div className="w-full bg-zinc-50 border border-zinc-200/80 rounded-2xl p-4 mb-5 text-left">
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-xs font-bold text-zinc-700 uppercase tracking-wider">
                      Tamaño / Ajuste al círculo
                    </label>
                  ...
                  <input 
                    ...
                    step="2"
                    value={loaderEscala}
                    onChange={(e) => setLoaderEscala(parseInt(e.target.value, 10))}
                    className="w-full accent-rose-600 h-2 bg-zinc-200 rounded-lg appearance-none cursor-pointer"
                  />
                  <div className="flex justify-between text-[10px] font-semibold text-zinc-400 mt-1">
                    <span>Pequeña (30%)</span>
                    <span>100% (Normal)</span>
                    <span>Grande (180%)</span>
                  </div>
                </div>

                {/* Botones de acción */}
                <div className="w-full flex flex-col gap-3">
                  <label className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-rose-50 border border-rose-200 text-rose-700 font-bold rounded-2xl hover:bg-rose-100 transition-colors cursor-pointer shadow-sm active:scale-95">
                    <Upload className="w-4 h-4 text-rose-600" />
                    <span>{loaderFile ? 'Cambiar imagen elegida...' : 'Seleccionar nueva imagen...'}</span>
                    <input 
                      type="file" 
                  ...
                  {/* Botón de Guardar (disponible si hay nueva imagen o si se ha cambiado el tamaño) */}
                  <button
                    type="button"
                    disabled={loaderStatus === 'saving'}
                    onClick={handleSaveLoaderImage}
                    className="w-full flex items-center justify-center gap-2 px-5 py-3 bg-red-600 text-white font-bold rounded-2xl hover:bg-red-700 transition-all shadow-md shadow-red-500/20 active:scale-95 disabled:opacity-50 cursor-pointer"
                  >
                    {loaderStatus === 'saving' ? (
                      <>
                        <Loader className="w-4 h-4 animate-spin" />
                        <span>Guardando cambios en la nube...</span>
                      </>
                    ) : (
                      <>
                        <Sparkles className="w-4 h-4" />
                        <span>Guardar y Aplicar (Imagen y Tamaño)</span>
                      </>
                    )}
                  </button>

                  {/* Botón para restablecer por defecto */}
                  <button
                    type="button"
                    disabled={loaderStatus === 'saving'}
                    onClick={handleResetLoaderImage}
                    className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-xs text-zinc-500 hover:text-zinc-800 hover:bg-zinc-100 rounded-xl transition-colors cursor-pointer"
                  >
                    <RotateCw className="w-3.5 h-3.5" />
                    <span>Restablecer imagen original (Racor por defecto)</span>
                  </button>
                  ...
                  {loaderStatus === 'success' && (
                    <div className="p-3 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl text-xs font-bold animate-in fade-in">
                      ✓ Imagen de inicio actualizada correctamente en todos los dispositivos.
                    </div>
                  )}
                  {loaderStatus === 'error' && (
                    <div className="p-3 bg-red-50 border border-red-200 text-red-800 rounded-xl text-xs font-bold animate-in fade-in">
                      Ocurrió un error al guardar la imagen. Revisa la conexión.
                    </div>
                  )}
                </div>
              </div>
