...
import { useNavigate, useLocation } from 'react-router-dom'; // Removed unused FileText, CheckCheck, Calendar, FileCheck, FileX
import { ArrowLeft, Save, Plus, Search, Edit, Trash2, MapPin, Layers, X, Copy, AlertTriangle, GripHorizontal, Upload, Download, Building2, UserCheck, FireExtinguisher, CalendarDays } from 'lucide-react';
...
export default function Centros() {
  ...
  const [categoriasSistema] = useState<{id: string, nombre: string}[]>(() => {
    // Initialized from localStorage, no need for useEffect
  ...
  const [tecnicos, setTecnicos] = useState<Tecnico[]>(() => { // Initialized from localStorage
    const saved = localStorage.getItem('firecheck_db_tecnicos');
    return saved ? JSON.parse(saved) : [];
  });
  ...
  const [empresas, setEmpresas] = useState<EmpresaData[]>(() => {
    try {
      const saved = localStorage.getItem('firecheck_db_empresas');
      return saved ? JSON.parse(saved) : [];
    } catch { return []; }
  });
  ...
  useEffect(() => {
    try {
      ...
      if (hasChanges) {
        localStorage.setItem('firecheck_db_centros', JSON.stringify(finalCentros)); // Removed redundant setCentros
  ...
  useEffect(() => {
    if (centros.length > 0 && location.state?.action === 'abrir-centro' && location.state?.centroId) {
      ...
      if (targetCentro) {
        ...
        setView('sistemas');
        navigate(location.pathname, { replace: true, state: { ...location.state, action: undefined } }); // Added location.pathname to deps
      }
    }
  ...
  useEffect(() => {
    const unsubscribe = subscribeTecnicos((items: Tecnico[]) => { // Added type for items
      setTecnicos(items);
  ...
  useEffect(() => {
    const unsubscribe = subscribeEmpresas((items: EmpresaData[]) => { // Added type for items
      setEmpresas(items);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      if ((form as any)._docId) {
        console.info('handleSave: updating existing centro, _docId=', (form as any)._docId, 'data=', newCentro); // Removed redundant console.info
        ...
        console.info('handleSave: adding new centro to Firestore, data=', newCentro); // Removed redundant console.info
        const created = await addCentro(newCentro);
        console.info('handleSave: addCentro returned', created); // Removed redundant console.info
  ...
  const handleSavePeriodicidad = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error guardando periodicidad en Firestore:', err); // Changed _e to err
  ...
  const handleSaveTecnicoAsignado = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error asignando técnico en Firestore:', err); // Changed _e to err
      alert('Error al guardar el técnico asignado en Firestore');
      return;
    }

    const updatedCentros = centros.map(c =>
      c.id === centroForTecnico.id
        ? { ...updatedCentro, _docId: (c as any)._docId || _docId }
        : c
    );
    saveToDB(updatedCentros);
    ...
    if (Array.isArray(storedPartes)) {
      const updatedPartes = storedPartes.map((parte: Parte) =>
        parte.centroId === centroForTecnico.id
          ? { ...parte, tecnicoId: selectedTecnicoId }
          : parte
      );
  ...
  const handleSaveEmpresaAsignada = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('Error asignando empresa en Firestore:', err); // Changed _e to err
      alert('Error al guardar la empresa asignada en Firestore');
      return;
    }

    const updatedCentros = centros.map(c =>
      c.id === centroForEmpresa.id
        ? { ...updatedCentro, _docId: (c as any)._docId || _docId }
        : c
    );
    saveToDB(updatedCentros);
    ...
    if (Array.isArray(storedPartes)) {
      const updatedPartes = storedPartes.map((parte: Parte) =>
        parte.centroId === centroForEmpresa.id
          ? { ...parte, empresaId: selectedEmpresaId }
          : parte
      );
      localStorage.setItem('firecheck_db_partes', JSON.stringify(updatedPartes));
    }
  ...
  }; // Removed duplicate handleSaveSistema
  ...
  const handleAddDesdeCatalogo = (e: React.FormEvent) => {
    ...
    const nuevosEquipos: EquipoInstalado[] = []; // Removed duplicate nuevosEquipos
    for (let i = 0; i < cantidadAñadir; i++) {
      // Calcular el número correlativo para el sistema seleccionado
      ...
      // startNum se recalcula en cada iteración sumando i para que sea correlativo
      ...
      nuevosEquipos.push({
        ...
        codigo: codigoNum.toString().padStart(2, '0'),
  ...
  const handleUpdateEquipo = (e: React.FormEvent) => {
    ...
    setIsEquipoModalOpen(false); // Removed duplicate setIsEquipoModalOpen
  };
  ...
  const sensors = useSensors(
    useSensor(PointerSensor, { // Removed duplicate useSensor
          ...
          {centros.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredCentros.map((c) => {
                const client = clientes.find(cl => cl.id === c.clienteId);
                const tecnicoAsignado = tecnicos.find(t => t.id === c.tecnicoId || t._docId === c.tecnicoId);
                const empresaAsignada = empresas.find(emp => emp._docId === c.empresaId);
                return (
                  <div key={c.id} className="bg-white p-3.5 rounded-3xl border-2 border-zinc-300 ring-4 ring-zinc-100 shadow-md shadow-zinc-200/60 hover:border-black hover:ring-zinc-200 hover:shadow-lg hover:shadow-zinc-300/70 transition-all flex flex-col h-full">
                    ...
                    <div className="pt-3 border-t border-zinc-100 flex flex-col gap-2">
                      ...
                      <button
                        onClick={() => {
                          setCentroForEmpresa(c);
                          setSelectedEmpresaId(c.empresaId || '');
                          setIsEmpresaModalOpen(true);
                        }}
                        className="w-full flex items-center justify-center gap-2 text-sm bg-blue-300 hover:bg-blue-400 text-blue-950 px-3 py-2 rounded-xl font-medium transition-colors shadow-sm"
                      >
                        <Building2 className="w-4 h-4" />
                        {empresaAsignada ? `Empresa: ${empresaAsignada.nombre}` : 'Asignar empresa'}
                      </button>
                      <button
                        onClick={() => {
                          setCentroForTecnico(c);
                          setSelectedTecnicoId(c.tecnicoId || '');
                          setIsTecnicoModalOpen(true);
                        }}
                        className="w-full flex items-center justify-center gap-2 text-sm bg-blue-500 hover:bg-blue-600 text-white px-3 py-2 rounded-xl font-medium transition-colors shadow-sm"
                      >
                        <UserCheck className="w-4 h-4" />
                        {tecnicoAsignado ? `Técnico: ${tecnicoAsignado.nombre} ${tecnicoAsignado.apellidos}` : 'Asignar técnico'}
                      </button>
                      <button 
                        onClick={() => openSistemas(c)} 
                        className="w-full flex items-center justify-center gap-2 text-sm bg-blue-700 hover:bg-blue-800 text-white px-3 py-2 rounded-xl font-medium transition-colors shadow-sm"
                      >
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 text-sm bg-blue-900 hover:bg-blue-950 text-white px-3 py-2 rounded-xl font-medium transition-colors shadow-sm"
                      >
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
              <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-blue-50/70">
                  <div>
                    <h2 className="text-lg font-bold text-zinc-900">Asignar técnico</h2>
                    <p className="text-xs text-zinc-500">{centroForTecnico.nombre}</p>
                  </div>
                  <button onClick={() => setIsTecnicoModalOpen(false)} className="p-2 text-zinc-400 hover:text-black hover:bg-white rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <form onSubmit={handleSaveTecnicoAsignado} className="p-6 space-y-5">
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-blue-900 flex items-center gap-2">
                      <UserCheck className="w-4 h-4" /> Técnico asignado al centro
                    </label>
                    <select
                      value={selectedTecnicoId}
                      onChange={e => setSelectedTecnicoId(e.target.value)}
                      className="w-full px-4 py-3 bg-blue-50/30 border border-blue-100 rounded-xl focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 outline-none"
                    >
                      <option value="">Sin técnico asignado</option>
                      {tecnicos.map(t => (
                        <option key={t._docId ?? t.id} value={t.id}>{t.nombre} {t.apellidos}</option>
                      ))}
                    </select>
                    <p className="text-xs text-zinc-500">
                      Esta asignación se aplicará también a los partes existentes de este centro.
                    </p>
                  </div>
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-[100]">
              <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl flex flex-col animate-in fade-in zoom-in-95 duration-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-rose-50/80">
                  <div>
                    <h2 className="text-lg font-bold text-zinc-900">Asignar empresa</h2>
                    <p className="text-xs text-zinc-500">{centroForEmpresa.nombre}</p>
                  </div>
                  <button onClick={() => setIsEmpresaModalOpen(false)} className="p-2 text-zinc-400 hover:text-black hover:bg-white rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                ...
                <form onSubmit={handleSaveEmpresaAsignada} className="p-6 space-y-5">
                  <div className="space-y-2">
                    <label className="text-sm font-semibold text-rose-950 flex items-center gap-2">
                      <Building2 className="w-4 h-4" /> Empresa asignada al centro
                    </label>
                    <select
                      value={selectedEmpresaId}
                      onChange={e => setSelectedEmpresaId(e.target.value)}
                      className="w-full px-4 py-3 bg-rose-50/30 border border-rose-100 rounded-xl focus:ring-2 focus:ring-rose-800/20 focus:border-rose-800 outline-none"
                    >
                      <option value="">Sin empresa asignada</option>
                      {empresas.map(emp => (
                        <option key={emp._docId} value={emp._docId}>{emp.nombre}{emp.cif ? ` (${emp.cif})` : ''}</option>
                      ))}
                    </select>
                    <p className="text-xs text-zinc-500">
                      Esta asignación se aplicará también a los partes existentes de este centro.
      ...
      <div className="min-h-screen bg-red-50/40 p-6 md:p-12">
        <div className="max-w-4xl mx-auto w-full">
          <button 
            ...
            className="text-sm font-medium text-red-600 hover:text-red-950 mb-8 flex items-center gap-2 transition-colors"
          >
          ...
          <div className="mb-8">
            <div className="mb-5">
              <span className="text-xs font-bold text-red-600 uppercase tracking-wider mb-1 block">
                {clientInfo?.nombre || 'Cliente'} &bull; {centroSeleccionado.nombre}
              </span>
              <h1 className="text-3xl font-bold text-red-950 tracking-tight flex items-center gap-3">
          ...
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEndSistemas}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {sistDelCentro.length === 0 ? (
                <div className="col-span-full text-center py-12 bg-white rounded-3xl border border-red-100 border-dashed">
                  <Layers className="w-12 h-12 text-red-200 mx-auto mb-3" />
                  <p className="text-red-900/50 font-medium">Este centro no tiene sistemas registrados.</p>
                </div>
              ) : (
                <SortableContext items={sistDelCentro.map(s => s.id)} strategy={rectSortingStrategy}>
                  {sistDelCentro.map(sist => {
                    ...
                    <SortableSistemaWrapper key={sist.id} sist={sist}>
                      ...
                      <div className="bg-white p-4 rounded-3xl border-2 border-red-100 shadow-sm hover:shadow-xl hover:border-red-400 transition-all flex flex-col group h-full relative overflow-hidden">
                        ...
                        <div className="flex justify-between items-start mb-3 relative z-10">
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-9 h-9 bg-red-50 rounded-xl flex items-center justify-center text-red-600 overflow-hidden relative z-10 shrink-0">
                      ...
                      <div className="min-w-0">
                        <span className="text-[10px] font-bold text-red-600 uppercase tracking-wider block mb-0.5">{sist.tipo}</span>
                        <div className="flex items-center gap-2">
                          <h3 className="font-bold text-red-950 text-base leading-tight truncate">{sist.familia || sist.tipo}</h3>
                          <span className="shrink-0 px-1.5 py-0.5 bg-red-100 text-red-800 text-[10px] font-mono font-bold rounded" title="Equipos registrados">
                            {equiposCount}
                          </span>
                        </div>
                      </div>
                  ...
                  {sist.descripcion && (
                    <div className="bg-red-50/50 p-3 rounded-xl border border-red-50 mt-2 relative z-10">
                      <p className="text-sm text-red-900/70">{sist.descripcion}</p>
                    </div>
                  )}
                  <div className="mt-4 pt-4 border-t border-red-50 relative z-10">
                    ...
                    {equiposDelSistema.length === 0 ? (
                      <p className="text-xs text-red-600/50 italic py-2 text-center bg-red-50/50 rounded-lg border border-red-50/50">Sin equipos</p>
                     ) : (
                          ...
                          return (
                            <div key={eq.id} className="flex justify-between items-center text-sm p-2 rounded-lg bg-white border border-slate-200 hover:border-slate-300 transition-colors">
                              <span className="font-medium text-slate-700 truncate pr-2 flex items-center gap-2">
                                <span className="px-2 py-0.5 bg-black text-white text-xs font-mono font-bold rounded-lg shadow-sm min-w-[28px] text-center">{eq.codigo || (i+1).toString().padStart(2, '0')}</span> 
            ...
            <div className="fixed inset-0 bg-red-950/20 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col max-h-[90vh]">
                <div className="px-6 py-4 border-b border-red-100 flex items-center justify-between bg-red-50/30 shrink-0">
                  ...
                  <button onClick={() => { setIsEquipoModalOpen(false); setFormEquipo({ id: '', centroId: '', sistemaId: '', codigo: '', nombre: '', ubicacion: '' }); setSelectedEquipoCatalogo(''); setIsClaseOtro(false); }} className="p-2 text-red-400 hover:text-red-700 hover:bg-white rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                    ...
                    <form onSubmit={handleUpdateEquipo} className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-sm font-semibold text-red-950">Nº orden</label>
                          <input type="text" value={formEquipo.codigo} onChange={e => setFormEquipo({...formEquipo, codigo: e.target.value.toUpperCase()})} className="w-full px-4 py-2 bg-red-50/50 border border-red-100 rounded-xl text-red-950" />
                        </div>
                        <div className="space-y-1.5 md:col-span-3">
                          <label className="text-sm font-semibold text-red-950">Nombre del Equipo</label>
                          <input type="text" value={formEquipo.nombre} onChange={e => setFormEquipo({...formEquipo, nombre: e.target.value})} className="w-full px-4 py-2 bg-red-50/50 border border-red-100 rounded-xl text-red-950" />
                        </div>
                      </div>
                      <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-red-800 uppercase">Placa / ID</label>
                          <input type="text" value={formEquipo.placa || ''} onChange={e => setFormEquipo({...formEquipo, placa: e.target.value})} className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs" maxLength={10} />
                        </div>
                        <div className="space-y-1.5 col-span-2">
                          <label className="text-[10px] font-bold text-red-800 uppercase">Ubicación</label>
                          <input type="text" value={formEquipo.ubicacion} onChange={e => setFormEquipo({...formEquipo, ubicacion: e.target.value})} className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs" />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          {sistemaSeleccionado && (sistemaSeleccionado.tipo || '').toUpperCase().includes('EXTINTOR') ? (
                            <div className="space-y-1">
                              <select 
                                ...
                                className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs outline-none focus:ring-2 focus:ring-red-500/20"
                              >
                              ...
                              {isClaseOtro && (
                                <input 
                                  ...
                                  className="w-full px-3 py-2 mt-1 bg-white border border-red-100 rounded-lg text-xs" 
                                />
                              )}
                            </div>
                          ) : (
                        ...
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-red-800 uppercase">Fabricante</label>
                          <input type="text" value={formEquipo.fabricante || ''} onChange={e => setFormEquipo({...formEquipo, fabricante: e.target.value})} className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs" />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-red-800 uppercase">FECHA FABRICACIÓN</label>
                          <input type="month" value={formEquipo.fechaFabricacion || ''} onChange={e => setFormEquipo({...formEquipo, fechaFabricacion: e.target.value})} className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs" placeholder="Selecciona mes y año" />
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-[10px] font-bold text-red-800 uppercase">FECHA ULTIMO RETIMBRE</label>
                          <input type="month" value={formEquipo.ultimoRetimbre || ''} onChange={e => setFormEquipo({...formEquipo, ultimoRetimbre: e.target.value})} className="w-full px-3 py-2 bg-red-50/30 border border-red-100 rounded-lg text-xs" placeholder="Selecciona mes y año" />
                        </div>
                      </div>
                    ...
                    <form onSubmit={handleAddDesdeCatalogo} className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <select
                          ...
                          className="w-full px-4 py-2.5 bg-red-50/50 border border-red-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-red-950 disabled:opacity-80 disabled:cursor-not-allowed"
                        >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select
                          ...
                          className="w-full px-4 py-2.5 bg-red-50/50 border border-red-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-red-950 disabled:opacity-60 disabled:cursor-not-allowed"
                        >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input
                          ...
                          className="w-full px-4 py-2.5 bg-red-50/50 border border-red-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-red-950"
                        />
                      </div>
                      <div className="pt-2 flex gap-3">
                        ...
                        <button type="submit" disabled={!selectedEquipoCatalogo} className="flex-1 px-4 py-2.5 text-white bg-red-600 hover:bg-red-700 disabled:opacity-50 rounded-xl font-medium transition-colors shadow-sm">
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                  ...
                  <button onClick={() => setIsSistemaModalOpen(false)} className="p-2 text-zinc-400 hover:text-black hover:bg-white rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <form onSubmit={handleSaveSistema} className="p-6 space-y-4">
                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-zinc-900">Familia / Nombre en este centro *</label>
                    <input
                      required type="text" value={formSistema.familia} onChange={e => setFormSistema({...formSistema, familia: e.target.value})}
                      className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-900/20 transition-all text-zinc-900"
                      placeholder="Ej: SISTEMA EXTINTORES NAVE 1"
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-sm font-semibold text-zinc-900">Descripción / Notas</label>
                    <textarea
                      value={formSistema.descripcion} onChange={e => setFormSistema({...formSistema, descripcion: e.target.value})}
                      className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-900/20 transition-all text-zinc-900 resize-none"
                      rows={3}
                      placeholder="Alguna observación sobre este sistema..."
                    />
                  </div>
                  <div className="pt-2 flex gap-3">
                    <button type="button" onClick={() => setIsSistemaModalOpen(false)} className="flex-1 px-4 py-3 rounded-xl font-bold text-zinc-500 hover:bg-zinc-100 transition-colors">Cancelar</button>
                    <button type="submit" className="flex-1 px-4 py-2.5 text-white bg-zinc-900 hover:bg-black rounded-xl font-medium transition-colors shadow-sm">Guardar Cambios</button>
                  </div>
                </form>
              </div>
            </div>
            ...
            <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
              <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                <div className="px-6 py-4 border-b border-zinc-100 flex items-center justify-between bg-zinc-50/50">
                  ...
                  <button onClick={() => setIsAddCatModalOpen(false)} className="p-2 text-zinc-400 hover:text-black hover:bg-white rounded-xl transition-colors">
                    <X className="w-5 h-5" />
                  </button>
                </div>
                <form onSubmit={handleAddSistemaFromCatalog} className="p-6 space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full px-4 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-zinc-900/20 transition-all text-zinc-900 disabled:opacity-60 disabled:cursor-not-allowed"
                    >
                      <option value="">
                  ...
                  <div className="pt-2 flex gap-3">
                    <button type="button" onClick={() => setIsAddCatModalOpen(false)} className="flex-1 px-4 py-3 rounded-xl font-bold text-zinc-500 hover:bg-zinc-100 transition-colors">Cancelar</button>
                    <button type="submit" disabled={!selectedCatIdForCentro} className="flex-1 px-4 py-2.5 text-white bg-zinc-900 hover:bg-black disabled:opacity-50 rounded-xl font-medium transition-colors shadow-sm">
                      Añadir al Centro
                    </button>
                  </div>
                </form>
