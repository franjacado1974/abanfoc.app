...
export default function Certificados() {
  ...
  const [clientes] = useState<any[]>(() => JSON.parse(localStorage.getItem('firecheck_db_clientes') || '[]')); // Initialized from localStorage
  const [centros] = useState<any[]>(() => JSON.parse(localStorage.getItem('firecheck_db_centros') || '[]')); // Initialized from localStorage
  const [sistemas] = useState<any[]>(() => JSON.parse(localStorage.getItem('firecheck_db_centro_sistemas') || '[]')); // Initialized from localStorage
  const [equipos, setEquipos] = useState<any[]>([]);
  const [tecnicos] = useState<any[]>(() => JSON.parse(localStorage.getItem('firecheck_db_tecnicos') || '[]')); // Initialized from localStorage
  ...
  useEffect(() => {
    const storedCertificados = JSON.parse(localStorage.getItem('firecheck_db_certificados') || '[]'); // Initialized from localStorage
    setCertificados(storedCertificados);
    // The other states are already initialized from localStorage in useState, no need to set them again here.
  ...
  const sortedAndFilteredCertificados = useMemo(() => { // Changed 'filtered' to 'const'
