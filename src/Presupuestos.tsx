...
import { Search, X, Download, Edit, Send, Trash2, Save, Package, Wrench, Type, Calculator, CheckCircle, Clock, Ban, ChevronDown, FileText, ArrowLeft, Plus, HardHat, Gauge, Check } from 'lucide-react';
import { 
...
export default function Presupuestos() {
  ...
  const handleConfirmarAceptacion = async () => {
    ...
    try {
      ...
      setPresupuestos(prev => prev.map(item => ((item as any)._docId === docId || item.id === p.id) ? { ...item, estado: 'Aprobado', pedidoId: codigoPedido } : item));
      ...
      const notaPedido = (p.notas && p.notas.trim()) ? p.notas.trim() : `Pedido ${codigoPedido} generado desde presupuesto ${codigoPresupuesto}`;
      ...
      setToastExito(`Presupuesto aceptado. Se ha generado ${codigoPedido} y derivado a ${tipoTrabajoSeleccionado}.`);
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full shadow-2xl border border-zinc-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="p-6 border-b border-zinc-100 flex items-start justify-between bg-gradient-to-r from-zinc-900 to-zinc-800 text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black tracking-tight">Presupuesto Aceptado</h3>
                  <p className="text-xs text-zinc-300 mt-0.5">Selecciona el tipo de trabajo para derivar el pedido</p>
                </div>
              </div>
              <button 
